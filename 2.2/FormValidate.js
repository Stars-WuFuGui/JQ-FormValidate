...
Validator.prototype = {
    loadValue: function () {
        ...
        for (var key in self.validates) {
            self.blured(self.form.find("*[name='" + key + "']:not([type='hidden'])"), 0);
        }
    },
    initForm: function (form) {
        ...
        form.find('input,select').each(function () {
            ...
            if(inputType == "date" || inputType == "file"){
                ...
                attr=attr+":";
                $(placeholderSpan).addClass("Select");
                text="请选择";
            }
            placeholderSpan.text(text + attr).addClass("placeholder").data('name', name).click(function () {
    ...
    loadingPlaceholder: function (form) {
        ...
        $(self.form).find('input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),select').on('focus', function () {
        ...
        $(self.form).find('input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),select').on('blur', function () {
    ...
    setValidate: function (name, validate) {
        ...
        $(".placeholder").each(function (index, element) {
            ...
            if (thisPlaceholder == name) {
                if ($(this).hasClass('dateAction')||$(this).hasClass('Select')){
